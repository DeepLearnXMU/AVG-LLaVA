...
class ModelArguments:
    ...
    vis_token_granularity: Optional[str] = field(default=None)
    unfreeze_mm_vision_tower: bool = field(default=False)
    image_grid_pinpoints: Optional[str] = field(default="[[336, 672], [672, 336], [672, 672], [1008, 336], [336, 1008]]")
    router_select: bool = field(default=False)
    tune_router: bool = field(default=False)
    margin: Optional[float] = field(default=0.1)
    rank_loss_weight: Optional[float] = field(default=1)
    ce_loss_weight: Optional[float] = field(default=1)
    filtered_text_num: Optional[int] = field(default=32)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            # print("image_file: ", image_folder+image_file) # delete
...
def train(attn_implementation=None):
    ...
    vis_token_granularity = getattr(model_args, 'vis_token_granularity', None)
    model.model.config.vis_token_granularity = list_of_integers(vis_token_granularity)
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.tune_router:
            ...
            for p in model.get_model().router.parameters():
                p.requires_grad = True
            # model.get_model().router.text_resample.pos_embed.requires_grad_(False)
